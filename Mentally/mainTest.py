...
print("\nAnteprima del DataFrame 'test':")
print(test.head())
print("\nInformazioni sul DataFrame 'test':")
print(test.info())
print("\Statistiche descrittive del DataFrame 'test':")
print(test.describe())
...
# --- Visualizzazione della Matrice di Correlazione ---
# Definisci la lista delle colonne numeriche *come l'avevi*, ma senza 'Depression'
numeric_cols = [
    'Age', 'Academic Pressure', 'Work Pressure', 'CGPA', 'Study Satisfaction',
    'Job Satisfaction', 'Sleep Duration', 'Work/Study Hours', 'Financial Stress',
    # 'Depression',  <-- rimosso
...
# Filtra solo le colonne presenti in df_clean
numeric_cols = [col for col in numeric_cols if col in df_clean.columns]

# Ora il corr non darà errori
corr_matrix = df_clean[numeric_cols].corr()
...
#X_test = df_clean
# # Scegli quale modello vuoi usare per la submission:
# # ad es. XGB Classifier ottimizzato
# model = joblib.load('best_model_XGB_Classifier.pkl')
# # Se vuoi la classe (0/1):
# y_pred_class = model.predict(X_test)

# # Se vuoi la probabilità di classe "1" (utile per soglie custom o ranking):
# y_pred_proba = model.predict_proba(X_test)[:, 1]

# # --- Creazione della submission ---
# # Scegli se esportare la classe o la probabilità:
# submission = pd.DataFrame({
#     'id': test_ids,
#     'Depression': y_pred_class       # oppure 'Depression_Prob': y_pred_proba
# })

# # Salva su file
# submission.to_csv('Mentally/submission.csv', index=False)
# print("Submission salvata su 'submission.csv'")
